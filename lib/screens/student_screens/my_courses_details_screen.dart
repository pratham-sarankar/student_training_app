...
class _MyCoursesDetailsScreenState extends State<MyCoursesDetailsScreen> {
  ...
  List<ChatMessage> _localMessages =
      []; // Add local messages list for immediate updates
  bool _isLoading = false;
  String? _courseChatId;
  StreamSubscription<QuerySnapshot>? _messagesSubscription;
  Timer? _debounceTimer; // Add debounce timer
  bool _isSendingMessage =
      false; // Add flag to track if we're sending a message
      ...
      if (!chatDoc.exists) {
        ...
        await FirebaseFirestore.instance
            ...
            .set({
              ...
              'lastMessage':
                  'Welcome to ${widget.course['title']}! How can I help you today?',
        ...
        await FirebaseFirestore.instance
            ...
            .set({
              ...
              'isActive': true,
              'userChatRef':
                  'users/${currentUser.uid}/course_chats/${widget.course['id']}',
            });
      } else {
        ...
        if (!globalChatDoc.exists) {
          ...
          await FirebaseFirestore.instance
              ...
              .set({
                ...
                'lastMessageTime':
                    chatDoc.data()?['lastMessageTime'] ?? Timestamp.now(),
                'lastSenderId': chatDoc.data()?['lastSenderId'] ?? 'instructor',
                'lastSenderName':
                    chatDoc.data()?['lastSenderName'] ?? 'Course Instructor',
  ...
  void _loadCourseNotes() {
    try {
      ...
      _courseNotesService
          .getCourseNotes(widget.course['id'])
          .listen(
            (notes) {
              if (mounted) {
                setState(() {
            ...
            children: [
              ...
              Expanded(
                child: IndexedStack(
                  ...
                  children: [_buildNotesTab(), _buildChatTab()],
                ),
              ),
            ],
          ),
  ...
  void _mergeLocalAndRemoteMessages() {
    ...
    for (final remoteMessage in _messages) {
      ...
      final isDuplicate = _localMessages.any(
        (local) =>
    ...
    // Debug logging
    print(
      'Merged messages - Local: ${_localMessages.length}, Remote: ${_messages.length}, Combined: ${combinedMessages.length}',
    );
  }
