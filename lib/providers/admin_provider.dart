...
class AdminProvider extends ChangeNotifier {
  ...
  Future<bool> createAdminUser(String email, String password, String firstName, String lastName) async {
    setState(isLoading: true, errorMessage: null);
  ...
  Future<void> addTraining(Training training) async {
    try {
      ...
      // Add to Firestore
      final docRef = await _firestore.collection('courses').add(training.toJson());
  ...
  Future<void> updateTraining(Training training) async {
    try {
      ...
      // Update in Firestore
      await _firestore.collection('courses').doc(training.id).update(training.toJson());
  ...
  Future<void> deleteTraining(String trainingId) async {
    try {
      print('AdminProvider: Starting training deletion - trainingId: $trainingId');
      ...
      if (trainingIndex != -1) {
        ...
        print('AdminProvider: Training removed from local state. New count: ${_trainings.length}');
        ...
        if (!_disposed) {
          ...
          if (remainingTraining) {
            print('AdminProvider: WARNING - Training still exists in local state after deletion!');
          } else {
            print('AdminProvider: Training successfully removed from local state');
          }
        }
  ...
  Future<void> addScheduleToTraining(String trainingId, TrainingSchedule schedule) async {
    try {
      print('AdminProvider: Starting schedule addition - trainingId: $trainingId, scheduleId: ${schedule.id}');
      print('AdminProvider: Current trainings count: ${_trainings.length}');
      ...
      if (trainingIndex != -1) {
        ...
        print('AdminProvider: Training has ${training.schedules.length} schedules before addition');
        
        // Add to local state first
        training.schedules.add(schedule);
        print('AdminProvider: Added schedule to local state. Training now has ${training.schedules.length} schedules');
        
        // Update in Firestore
        final updatedSchedules = training.schedules.map((s) => s.toJson()).toList();
        print('AdminProvider: Updating Firestore with ${updatedSchedules.length} schedules');
  ...
  Future<void> uploadNote(String trainingId, String scheduleId, Note note) async {
    try {
      final trainingIndex = _trainings.indexWhere((t) => t.id == trainingId);
      if (trainingIndex != -1) {
        ...
        if (scheduleIndex != -1) {
          ...
          // Update in Firestore
          final updatedSchedules = _trainings[trainingIndex].schedules.map((s) => s.toJson()).toList();
          await _firestore.collection('courses').doc(trainingId).update({
            'schedules': updatedSchedules,
          });
  ...
  Future<void> sendMessage(String trainingId, String scheduleId, Message message) async {
    try {
      final trainingIndex = _trainings.indexWhere((t) => t.id == trainingId);
      if (trainingIndex != -1) {
        final scheduleIndex = _trainings[trainingIndex]
  ...
  Future<void> deleteScheduleFromTraining(String trainingId, String scheduleId) async {
    try {
      print('AdminProvider: Starting schedule deletion - trainingId: $trainingId, scheduleId: $scheduleId');
      print('AdminProvider: Current trainings count: ${_trainings.length}');
      ...
      if (trainingIndex != -1) {
        final training = _trainings[trainingIndex];
        print('AdminProvider: Training has ${training.schedules.length} schedules before deletion');
        ...
        print('AdminProvider: Removed ${initialCount - finalCount} schedules from local state');
        print('AdminProvider: Training now has ${training.schedules.length} schedules');
  ...
  Future<void> updateScheduleInTraining(String trainingId, TrainingSchedule updatedSchedule) async {
    try {
      print('AdminProvider: Starting schedule update - trainingId: $trainingId, scheduleId: ${updatedSchedule.id}');
      print('AdminProvider: Current trainings count: ${_trainings.length}');
      ...
      if (trainingIndex != -1) {
        final training = _trainings[trainingIndex];
        print('AdminProvider: Training has ${training.schedules.length} schedules');
        
        final scheduleIndex = training.schedules.indexWhere((s) => s.id == updatedSchedule.id);
        ...
        if (scheduleIndex != -1) {
          ...
          print('AdminProvider: Schedule not found with ID: ${updatedSchedule.id}');
        }
      } else {
        print('AdminProvider: Training not found with ID: $trainingId');
      }
